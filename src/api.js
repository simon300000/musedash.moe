...
export const mdmcGetRank = async ({ id: i, difficulty }) => (await getMDMC(`rank/${i}/${difficulty}`)).map(([acc, score, lastRank, nickname, id, character, elfin]) => ({ acc, score, lastRank, nickname, id, character, elfin, url: `/mdmc/player/${id}` }))
